...
{ 
    ...
    await context.storageState({path: 'state.json'}); //First time you login and save the storage state(cookies) in a file.
    Webcontext = await browser.newContext({storageState: 'state.json'}); // Using/Injecting the saved cookies, you can invoke new browser context and use that context for all your tests. This will avoid the need to login before each test and save time.
     

});

test('Page Item List test', async ()=>
{
